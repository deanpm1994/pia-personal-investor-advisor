"""Confirm staged imports atomically and idempotently.
...
down_revision: str | Sequence[str] | None = "20260719_05"
branch_labels: str | Sequence[str] | None = None
depends_on: str | Sequence[str] | None = None


def upgrade() -> None:
    """Link confirmed events to imports and expose one atomic confirmation operation."""
    op.execute(
        """
        ALTER TABLE public.financial_events
            ADD COLUMN staged_import_id uuid,
            ADD CONSTRAINT financial_events_staged_import_owner_fk
                FOREIGN KEY (staged_import_id, user_id)
                REFERENCES public.staged_imports (id, user_id)
                ON DELETE RESTRICT
        """
    )
    op.execute(
        """
        CREATE INDEX financial_events_staged_import_id_idx
            ON public.financial_events (staged_import_id)
            WHERE staged_import_id IS NOT NULL
        """
    )
    op.execute(
        """
        CREATE FUNCTION public.confirm_staged_import(p_staged_import_id uuid)
    ...
    op.execute("REVOKE ALL ON FUNCTION public.confirm_staged_import(uuid) FROM PUBLIC")
    op.execute(
...
def downgrade() -> None:
    """Remove the confirmation operation and its ledger traceability column."""
    op.execute("DROP FUNCTION public.confirm_staged_import(uuid)")
    op.execute("DROP INDEX public.financial_events_staged_import_id_idx")
    op.execute(
        "ALTER TABLE public.financial_events "
        "DROP CONSTRAINT financial_events_staged_import_owner_fk"
    )
    op.execute("ALTER TABLE public.financial_events DROP COLUMN staged_import_id")
